/*
* Generates the equilibrium average M, E, C, Chi (without errorbars) at a certain L (you have to change this parameter) and at 20 different temperatures
*/
...
int main(int argc, const char * argv[]) {
	...
	ofstream filemc;
	filemc.open("traj_25_t.txt",ios::out);
	filemc.precision(10);
	

	for(int Tt = 0; Tt < 20; Tt++){ 
	
		kbT = 0.8*T_critic + 0.05*Tt;
		 
		setVectorToZero(M, nx * ny);
		setVectorToZero(E, nx * ny);

		for(int R = 0; R < realiz; R++){ 
			...
			for(long int t=0; t < nsteps; t++){
				...
				// Keep trace of energy and magnetisation
		    		M[t] =  magnetization(nx, ny, spins, index)/realiz; // ERA QUESTO L?ERRORE?????????
		...
		cout << E_ave/(nx*ny)  << endl;
		filemc << kbT << "\t" << E_ave/(nx*ny) << "\t" << M_ave/(nx*ny) << "\t"  << C_ave  << "\t" << Chi_ave << "\t" <<'\n';
