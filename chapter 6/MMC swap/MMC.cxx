...
vector<double> init_beta_list(double beta_i, double beta_f, int n_chains){
	...
	double delta = (beta_f - beta_i) / (n_chains-1); //step size delta between each element
	
	for (int i = 0; i < n_chains; i++) {
		beta_list.push_back(beta_i + i * delta);
		cout << beta_i + i * delta << endl;
	}
...
int main(int argc, const char * argv[]) {
	...
	int n_chains = 9 ;  // number of temperatures considered
	...
	int count_true = 0; // record the swapping rate at certain time t
	...
	for (int i = 0; i < n_chains; ++i) {
		...
		setVectorToOne(spins[i], nx * ny);
		//initializeSpins(nx, ny, spins[i], index, mt_generator, uniform_dist);
	}
    	
        
	// inverse temperature list
	//vector<double> beta_list = init_beta_list(beta_i, beta_f, n_chains);
	...
	ofstream filemc1;
	ofstream filemc2;
	filemc1.open("E_9_80_Tc2.txt",ios::out);
	filemc1.precision(10);
	filemc2.open("M_9_80_Tc2.txt",ios::out);
	filemc2.precision(10);	
	
	cout << "choosing temperature to plot: " << 1/beta_list[5] << endl;
	...
	cout << " ***** end burnt in *****" << endl;
		...
		for (int i = 0; i < n_chains; ++i) {
        	...
        	// Attempt swaps between adjacent chains every n - MC steps
        	if (t%6 == 0){
			n_swap = 0;
			setVectorToFalse(chain_swapped, n_chains);

			for (int kk = 0; kk < n_chains; ++kk) {

				/*for (int kkk = 0; kkk < n_chains; ++kkk) {
					cout << chain_swapped[kkk] << endl;
				}*/
			
				
				// choose a random chain to attemp the swap
				k1 = uniform_swap(mt_generator);
				//cout << " k1 --> " << k1 <<endl;
				
				// Choose the adjacent chain
				if (k1 == 0) {
				    // only swap with the next one
				    k2 = k1 + 1;
				} else if (k1 == n_chains-1) {
				    // only swap with the previous one
				    k2 = k1 - 1;
				} else {
				    // otherwise, choose randomly between the previous and next chains
				...
				//cout << " k2 --> " << k2 << endl;
		    		...
		    		// Check if both k1 and k2 are not in the list of swapped chains
		    		if (chain_swapped[k1] != true && chain_swapped[k2]!= true) {
		    		
					// compute the acceptance_prob of swap between adjacent chains
					diff_ene = E[k1][t] - E[k2][t];
					acceptance_prob = exp((beta_list[k1] - beta_list[k2]) * diff_ene);
					u = uniform_dist(mt_generator);
						
					// attempt swap
					if (acceptance_prob > 1.0 || u < acceptance_prob) {
			
						// Swap configurations between chains k1 and k2
						n_swap += 1 ;
						//cout << " Swap accepted btw chains: " << k1 << " and " << k2 << endl;
						swap(configPointers[k1],configPointers[k2]);
						//swap(M[k1][t], M[k2][t]);
						//swap(E[k1][t], E[k2][t]);
						
						// Set the flag indicating that these chains have swapped
						chain_swapped[k1] = true;
						chain_swapped[k2] = true;

					}
				
				}
				
				// Record the number of swaps for the current timestep
	    			swap_rate[t] = n_swap;
	...
	ofstream filemc;
	filemc.open("swap_rate.txt",ios::out);
	filemc.precision(10);
	for(long int t = 0; t < nsteps + burnInSteps; 5*t++){
		filemc << swap_rate[t] << endl;
	}

	


	// Deallocate memory for E, M, and spins
	/*for (int i = 0; i < n_chains; ++i) {
	    //delete[] E[i];
	    delete[] M[i];
	    delete[] spins[i];
